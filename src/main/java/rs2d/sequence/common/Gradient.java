...
/**
 * Class Gradient: small Common Version 2.0S
 */
...
//        double ratioA2 = (W1 + 2 * W2 + 2 * delta) / (W1 + W2);
    ...
    public void reoderPhaseEncoding(TransformPlugin plugin) {
        int[] kpos; // kpos = (i,j,k,l)
        steps = plugin.getTransformSize()[1];
        double[] newTable = new double[steps];
        for (int scan2D = 0; scan2D < steps; scan2D++) { // 3D acquisition: a scan 2D -> acquisition of a k-space line of coordinate (j,k)
            kpos = plugin.transf(0, scan2D, 0, 0); // f(scan2D) = f(i_phase_step) = (j,k)
            newTable[scan2D] = amplitudeArray[kpos[1]]; // j: k-space coordinate driven by the gradient along phase axis
        }
        amplitudeArray = newTable;
        steps = amplitudeArray.length;
    }
    ...
    public void reoderPhaseEncoding3D(TransformPlugin plugin) {
        ...
        for (int scan2D = 0; scan2D < steps; scan2D++) { // 3D acquisition: a scan 2D -> acquisition of a k-space line of coordinate (j,k)
            ...
            newTable[scan2D] = amplitudeArray[kpos[2]]; // k: k-space coordinate driven by the gradient along slice axis
        }
        amplitudeArray = newTable;
        steps = amplitudeArray.length;
    }
